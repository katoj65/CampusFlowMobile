-- Checkout as a single atomic server-side function instead of two separate
-- client round trips (insert orders, then insert order_items) that could
-- leave a dangling empty order if the second insert failed. Total is now
-- computed from the cart_items actually stored server-side rather than
-- trusted from the client. security definer is required to also clear the
-- user's cart_items as part of the same transaction; user_id is always
-- taken from auth.uid(), never a parameter, so a caller can only ever act
-- on their own cart/orders regardless of the elevated privileges.
